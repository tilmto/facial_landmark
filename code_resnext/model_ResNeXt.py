...
class ResNeXt152:
	def __init__(self,images,ground_truth,points_num=106,is_training=True,img_size=256,scope='ResNeXt'):
		self.images = images
	...
	def residual_module(self,x,out_planes,stride=1,is_training=True,scope='residual',cardinality=32):
		...
		with tf.variable_scope(scope):
			...
			if in_planes != out_planes or stride != 1:
				card.append(slim.conv2d(orig_x,out_planes,[1,1],stride,padding='SAME',activation_fn=None,scope='downsample'))
			else:
				card.append(orig_x)

			for i in range(cardinality):
				x = slim.conv2d(orig_x,4,[1,1],stride,padding='SAME',activation_fn=None,scope='rconv1_'+str(i+1))
				x = tf.nn.relu(self.batch_norm(x,is_training,scope='rbn1_'+str(i+1)))
				x = slim.conv2d(x,4,[3,3],1,padding='SAME',activation_fn=None,scope='rconv2_'+str(i+1))
				x = tf.nn.relu(self.batch_norm(x,is_training,scope='rbn2_'+str(i+1)))
				x = slim.conv2d(x,out_planes,[1,1],1,padding='SAME',activation_fn=None,scope='rconv3_'+str(i+1))
				card.append(x)
...
if __name__ == '__main__':
	...
	with tf.Session() as sess:
		...
		resnet = ResNeXt152(images,ground_truth)
