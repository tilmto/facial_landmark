...
class ACNet(object):
    def __init__(self, scope, session):
        ...
        w=10.0
        epsilon=2.0
        x = self.model.landmark_output - self.landmarks
        ...
        self.loss_detection = tf.reduce_mean(tf.reduce_sum(losses, axis=[1, 2]), axis=0)       

        with tf.variable_scope("opt_plus"):
            self.optimizer = tf.train.MomentumOptimizer(0.00003,0.9).minimize(self.loss_detection)
...
def build_worker(session,is_training,model_name='my_model_best'):
    ...
    if not is_training:
        if flags.load_model:
            ...
            variables_to_restore = [v for v in variables if v.name.find('opt_plus')==-1] 
            saver2 = tf.train.Saver(variables_to_restore,max_to_keep=50)            
            saver2.restore(session,model_name)
