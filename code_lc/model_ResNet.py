...
class ResNet152:
	def __init__(self,images,ground_truth,points_num=106,is_training=True,img_size=256,scope='ResNet'):
		self.images = images
	...
	def build_model(self,scope,is_training=True):
		...
		with tf.variable_scope(scope):
			...
			x = self.residual_module(x,512,2,is_training,'residual2048_1',locally_connected = True)
			for i in range(2):
				x = self.residual_module(x,512,1,is_training,'residual2048_'+str(i+2),locally_connected = True)

			with tf.variable_scope('channel_fusion'):
				x = tf.contrib.keras.layers.LocallyConnected2D(self.points_num,[1,1],1,'valid',activation=None)(x)

			x = tf.nn.relu(self.batch_norm(x,is_training,scope='bn2'))

			with tf.variable_scope('compress'):
				x = tf.contrib.keras.layers.LocallyConnected2D(self.points_num,[4,2],2,'valid',activation=None)(x)
	...
	def residual_module(self,x,out_planes,stride=1,is_training=True,scope='residual',locally_connected=False):
		in_planes = x.shape[-1].value
		orig_x = x

		if locally_connected:
			with tf.variable_scope(scope):
				with tf.variable_scope('rconv1'):
					x = tf.contrib.keras.layers.LocallyConnected2D(int(out_planes/4),[1,1],stride,'valid',activation=None)(x)

				x = tf.nn.relu(self.batch_norm(x,is_training,scope='rbn1'))
				
				with tf.variable_scope('rconv2'):
					x = tf.pad(x,[[0,0],[1,1],[1,1],[0,0]])
					x = tf.contrib.keras.layers.LocallyConnected2D(int(out_planes/4),[3,3],1,'valid',activation=None)(x)

				x = tf.nn.relu(self.batch_norm(x,is_training,scope='rbn2'))

				with tf.variable_scope('rconv3'):
					x = tf.contrib.keras.layers.LocallyConnected2D(out_planes,[1,1],1,'valid',activation=None)(x)

				if in_planes != out_planes or stride != 1:
					with tf.variable_scope('downsample'):
						orig_x = tf.contrib.keras.layers.LocallyConnected2D(out_planes,[1,1],stride,'valid',activation=None)(orig_x)
...
if __name__ == '__main__':
	...
	with tf.Session() as sess:
		...
		resnet = ResNet152(images,ground_truth)
