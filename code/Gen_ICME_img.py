    ...
    while i<BATCH_SIZE:
        ...
        if fname.endswith('.png') or fname.endswith('.jpg'):
            ...
            if fname.find('IBUG')!=-1 or fname.find('ibug')!=-1:
                ...
                if rand_seed4<0.3:
                    cut_img = salt_noise(cut_img)
                elif rand_seed4<0.6:
                    cut_img = gaussian_blur(cut_img)
                '''
                ...
                if rand_seed<0.1:
                    ...
                    cut_img, ground_truth_frame=rot(cut_img, ground_truth_frame, 70*(random.random()-0.5))
                else:
