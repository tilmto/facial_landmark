...
from model_ResNet_tiny import ResNet152_tiny
from model_ResNet_group import ResNet152_group
from model_ResNet2 import ResNet101_HM
from model_ResNet3 import ResNet101_HMN
import copy
...
class ACNet(object):
    def __init__(self, scope, session):
        ...
        #################################convert 21 ponts to 8 points#################################
        self.is_training=tf.placeholder(tf.bool, name='is_training')
        self._build_net(self.state_tensor)
        #################################calculate loss#################################
        self.loss_detection = self.model.loss_detection
        self.optimizer = self.model.optimizer

        #self.loss_detection = tf.reduce_mean(tf.square((self.model.landmark_output1 - self.landmarks)))
    ...
    def _build_net(self, scope):
        ...
        if flags.model == '1':
            ...
            self.model=ResNet152_group(self.state_tensor, self.landmarks, POINT_NUMS, self.is_training)

        elif flags.model == '3':
            self.model=ResNet152_tiny(self.state_tensor, self.landmarks, POINT_NUMS, self.is_training)

        elif flags.model == '4':
            self.model=ResNet101_HM(self.state_tensor, self.landmarks, POINT_NUMS, self.is_training)

        elif flags.model == '5':
            self.model=ResNet101_HMN(self.state_tensor, self.landmarks, POINT_NUMS, self.is_training)
    ...
    with tf.Graph().as_default(), tf.Session(config=config) as session:
        ...
        for ep_idx in range(MAX_GLOBAL_EP if is_training else 1):
            ...
            while Gen_ICME_img.END_OF_ONE_EPOCH==False:
                #Below: ICME Data
                state_tensor,coord_labels=Gen_ICME_img.get_ICME_img(H,W,use_random_sample=False if ep_idx==0 else True)                  
                feed_dict_a={
