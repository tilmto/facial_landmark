...
class ResNet152_group:
	...
	def build_model(self,scope,is_training=True):
		...
		with tf.variable_scope(scope):
			...
			hg_out = tf.unstack(x,axis=-1)

			coord_list = []

			for p_idx in range(self.points_num):
				with tf.variable_scope('point'+str(p_idx)):
					if p_idx<33:
						net = slim.convolution(tf.expand_dims(hg_out[p_idx],axis=-1), 1, [2,2], 2, padding='SAME', scope='conv1',activation_fn=tf.nn.relu)
						net = slim.pool(net, [2, 2], 'MAX', stride=2, padding='SAME', scope='pool1')                            
						net = self.flatten_convolution(net)
						net = tf.layers.dense(net,50,activation=tf.nn.relu,use_bias=False)
						coord_a_point = tf.layers.dense(net,2,activation=None,use_bias=False)
					else:
						net = slim.convolution(tf.expand_dims(hg_out[p_idx],axis=-1), 2, [2,2], 1, padding='SAME', scope='conv1',activation_fn=tf.nn.relu)
						net = slim.pool(net, [2, 2], 'MAX', stride=2, padding='SAME', scope='pool1')                            
						net = slim.convolution(net, 2, [2,2], 1, padding='SAME', scope='conv2',activation_fn=tf.nn.relu)
						net = slim.pool(net, [2, 2], 'MAX', stride=2, padding='SAME', scope='pool2')                     
						net1 = slim.convolution(net, 2, [2,2], [2,2], padding='SAME', scope='conv3',activation_fn=None)
						coord_a_point = slim.pool(net1, [2, 2], 'MAX', stride=2, padding='SAME', scope='pool3')
				
					coord_list.append(self.flatten_convolution(coord_a_point))

			self.landmark_output = tf.stack(coord_list,axis=1,name='landmark')
