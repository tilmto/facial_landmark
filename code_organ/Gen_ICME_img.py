...
def flip_img(img,ground_truth_frame,mask):
    img_flip = cv.flip(img,1)
    gt = copy.deepcopy(ground_truth_frame)
    mk = copy.deepcopy(mask)
    ...
    return img_flip,np.array(gt_flip,np.float32),np.array(mk_flip,np.float32)
...
left_eye = list(range(66,75))+[104]
right_eye = list(range(75,84))+[105]
nose = list(range(51,66))
mouth = list(range(84,104))

mask_left_eye = np.array([True for i in range(106)])
mask_right_eye = np.array([True for i in range(106)])
mask_nose = np.array([True for i in range(106)])
mask_mouth = np.array([True for i in range(106)])

for item in left_eye:
	mask_left_eye[item]=False
for item in right_eye:
	mask_right_eye[item]=False
for item in nose:
	mask_nose[item]=False
for item in mouth:
	mask_mouth[item]=False

def get_organ_bound(img,ground_truth_frame,mask):
    neg_mask = list(map(lambda x:not x,mask))
    gt_organ = ground_truth_frame[neg_mask]
    return get_face_bound(gt_organ)


def rand_cover_organ(img,ground_truth_frame):
	...
	rand_list = random.sample(list(range(4)),cover_num)
	mask_list = []
	...
	for r in rand_list:
		...
		left,right,top,bottom = list(map(int,get_organ_bound(img,ground_truth_frame,mask)))

		fill_color = img[min(255,bottom),max(0,left)]

		for i in range(left-3,right+3):
			for j in range(top-3,bottom+3):
				if i<256 and j<256:
					img[j,i] = fill_color

		mask_list.append(mask)

	mask_all = np.array([True for i in range(106)])
	...
	for item in mask_all:
		if item:
			mask_final.append([1,1])
		else:
			mask_final.append([0,0])

	return img,np.array(mask_final)
...
def get_ICME_img(H=256,W=256,use_random_sample=False):
    ...
    while i<BATCH_SIZE:
        ...
        if fname.endswith('.png') or fname.endswith('.jpg'):
            ...
            if fname.find('IBUG')!=-1 or fname.find('ibug')!=-1:
                ...
                cut_img,mask = rand_cover_organ(cut_img,ground_truth_frame) 

                #draw_points(cut_img,ground_truth_frame)
                ...
                img_flip,gt_flip,mk_flip = flip_img(s_input,ground_truth_frame,mask)
    ...
    return np.array(state_tensor),np.array(coord_labels),np.array(mask_list)
